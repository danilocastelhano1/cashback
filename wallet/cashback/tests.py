...
class RentalTests(APITestCase):
    def setUp(self) -> None:
        ...
        self.user = User.objects.create_user(username=settings.USERNAME, password=settings.PASSWORD)
        self.credentials = base64.b64encode(f'{settings.USERNAME}:{settings.PASSWORD}'.encode('utf-8'))
    ...
    def test_should_error_when_passing_invalid_total(self):
        ...
        resp = self.client.post("/api/cashback/", data=self.data, format="json")
        self.assertEqual(resp.status_code, 400)
        self.assertEqual(resp.json()["products"][0], "Total does not match with the sum of products")
    ...
    def test_should_error_when_passing_invalid_type(self):
        ...
        resp = self.client.post("/api/cashback/", data=self.data, format="json")
        self.assertEqual(resp.status_code, 400)
        self.assertEqual(resp.json()["products"][0]["type"][0], '"E" is not a valid choice.')
